...
function PhotoArea({ fotoUrl, editing, onUpload }) {
  ...
  if (!editing) {
    return fotoUrl ? (
      <img
        ...
        alt=""
        style={{ width: '100%', aspectRatio: '4/3', objectFit: 'cover', borderRadius: '6px 6px 0 0', display: 'block' }}
      />
    ) : (
  ...
  return (
    <div
      ...
      style={{
        ...
        border: fotoUrl ? 'none' : '2px dashed var(--border)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        cursor: 'pointer', position: 'relative', overflow: 'hidden',
      }}
    >
      {fotoUrl && (
        <img src={`${fotoUrl}?t=${Date.now()}`} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', inset: 0 }} />
      )}
      <div style={{
        position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.45)',
        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '4px',
        opacity: fotoUrl ? 0 : 1, transition: 'opacity 0.15s',
      }}
        onMouseEnter={e => e.currentTarget.style.opacity = 1}
        onMouseLeave={e => e.currentTarget.style.opacity = fotoUrl ? 0 : 1}
      >
        <span style={{ fontSize: '1.4rem' }}>📷</span>
        <span style={{ color: '#fff', fontSize: '0.78rem', fontWeight: 600 }}>{fotoUrl ? 'Foto wijzigen' : 'Foto toevoegen'}</span>
      </div>
...
function WerkstukCard({ item, onSaved, onDeleted }) {
  ...
  function cancel() { setForm({ vak: item.vak, gemaakt_bij: item.gemaakt_bij, datum: item.datum, trots_omdat: item.trots_omdat }); setPending(null); setPreview(null); setEditing(false) }

  const displayFoto = previewUrl || (fotoUrl ? fotoUrl : null)
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '8px', overflow: 'hidden', boxShadow: 'var(--shadow-sm)', display: 'flex', flexDirection: 'column' }}>
      <PhotoArea fotoUrl={displayFoto} editing={editing} onUpload={pickPhoto} />
      ...
      <div style={{ padding: '14px 16px', flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {editing ? (
          ...
          <>
            {item.vak && <div><span style={{ fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-dim)', fontFamily: 'var(--title)' }}>Vak</span><div style={{ fontWeight: 600, fontSize: '0.92rem', marginTop: '1px' }}>{item.vak}</div></div>}
            {item.gemaakt_bij && <div><span style={{ fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-dim)', fontFamily: 'var(--title)' }}>Dit heb ik gemaakt bij</span><div style={{ fontSize: '0.85rem', marginTop: '1px' }}>{item.gemaakt_bij}</div></div>}
            {item.datum && <div><span style={{ fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-dim)', fontFamily: 'var(--title)' }}>Datum</span><div style={{ fontSize: '0.85rem', marginTop: '1px' }}>{item.datum}</div></div>}
            {item.trots_omdat && <div><span style={{ fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-dim)', fontFamily: 'var(--title)' }}>Ik ben hier trots op omdat</span><div style={{ fontSize: '0.85rem', color: 'var(--text-soft)', marginTop: '1px', lineHeight: 1.5 }}>{item.trots_omdat}</div></div>}
...
function AddForm({ onAdded, onCancel }) {
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1.5px solid var(--primary)', borderRadius: '8px', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
      <PhotoArea fotoUrl={previewUrl} editing onUpload={pickPhoto} />
  ...
  return (
    <>
      <div className="page-header">
        ...
        {!adding && (
          <button className="btn btn-primary" onClick={() => setAdding(true)}>+ Nieuw werkstuk</button>
        )}
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
        {adding && (
          <AddForm onAdded={handleAdded} onCancel={() => setAdding(false)} />
        )}
        {items.map(item => (
          <WerkstukCard key={item.id} item={item} onSaved={handleSaved} onDeleted={remove} />
        ))}
      </div>
